...
1. **A 4-node Kubernetes cluster** running on Talos Linux (`homme-cluster`), defined in `talos/` and `cluster/`.
...
- **Kubernetes nodes**: 3 × Minisforum MS-A2 (`ser-msa2cp1`, `ser-msa2cp2`, `ser-msa2cp3`), all control-plane, with a shared VIP (API endpoint `https://<vip>:6443`; node IPs and VIP live in `env.sops.yaml` as `TALOS_NODE_*_IP` / `TALOS_VIP`). Scheduling on control planes is allowed. Plus one dedicated worker: 1 × Minisforum UN100C Intel N100 (`ser-n100wk1`, NIC `enp3s0`, Talos extensions `intel-ucode` + `i915`) running the Frigate NVR with 2 USB Coral TPUs. It carries label `role=nvr` and taint `dedicated=nvr:NoSchedule` so only Frigate (plus system DaemonSets) schedules there, and `node.longhorn.io/create-default-disk=false` so Longhorn ignores its disk. Gotchas: k8s NodeRestriction blocks Talos `machine.nodeTaints` on workers (apply the taint once with `kubectl taint` at enrollment — the config stays the source of truth), Cilium `devices` must list every node's NIC name, and the same holds for the `interfaces` list of the `CiliumL2AnnouncementPolicy` (a node holding the announce lease for a LoadBalancer IP whose NIC is missing there silently never answers ARP for that IP), and Longhorn volumes cannot attach on it (its manager DaemonSet doesn't run there) — use node-local `hostPath` for pod state pinned to it.
...
│   └── apps/              #   platform/, iot/, media/, ai/ (shared AI runtimes: ollama)
...
- **GitOps engine**: FluxCD. The cluster syncs from `ssh://git@github.com/homme-ar/infra.git` branch `main` (see `cluster/flux-system/gotk-sync.yaml`). This is a **public repository** — see Security. Reconciliation is also push-based: a GitHub webhook (`push` events) hits `flux-webhook.${CLUSTER_DOMAIN}` → `Receiver/flux-system` (manifests in `cluster/flux-system/webhook/`, HMAC token in the SOPS-encrypted `secret.yaml`) → the `flux-system` GitRepository reconciles immediately; the 10m polling interval remains as fallback. NOTE: the `webhook-receiver` endpoint sits behind the Gateway, and Cilium Envoy upstream connections carry the reserved `ingress` identity — the `CiliumNetworkPolicy/allow-gateway-webhooks` (same directory) must allow `fromEntities: [ingress, host, remote-node]` or same-node backends get 503s (the flux-bootstrap `allow-webhooks` NetworkPolicy only allows pod traffic).
- **CNI / networking**: Cilium (HelmRelease, kube-proxy replacement, L2 announcements, Hubble) + Gateway API (`gatewayClassName: cilium`, central `Gateway` named `envoy-gateway` in `kube-system`, TLS terminated with the `${CLUSTER_DOMAIN_SLUG}-tls` cert). Apps expose HTTP via `HTTPRoute` resources, not Ingress. Every app HTTPRoute must pin `sectionName: https` in its `parentRefs` so it only attaches to the TLS listener; the `http` listener (port 80) is served exclusively by the global `http-to-https-redirect` HTTPRoute in `cluster/infrastructure/networking/gateway/` (301 RequestRedirect to HTTPS).
- **Storage**: Longhorn (default `${STORAGE_CLASS}` = `longhorn`; `longhorn-singlenode` single-replica class for databases). A dedicated Kingston NVMe disk per node is reserved for Longhorn (see `talos/patches/global/storage.yaml`). After any Longhorn chart upgrade, verify the running **v2 instance-managers** actually moved to the new version (`kubectl -n longhorn-system get instancemanagers.longhorn.io` — they are only replaced when drained to zero instances; see `docs/BACKUPS.md` troubleshooting).
...
- **Config reload**: stakater/Reloader (`cluster/infrastructure/security/reloader/`, Helm chart) restarts annotated workloads when their ConfigMaps/Secrets change. Opt-in per workload via `reloader.stakater.com/auto: "true"` on the Deployment/StatefulSet metadata (currently: `homepage`, `gatus`, `frigate`). Annotate any workload whose config is mounted via subPath or consumed as env vars and changes frequently.
- **TLS**: cert-manager with Let's Encrypt (Cloudflare DNS-01), issuers in `cluster/infrastructure/security/cert-manager-config/`.
- **Observability**: kube-prometheus-stack, metrics-server, node-feature-discovery, Gatus (`observability/gatus`, status page at `status.${CLUSTER_DOMAIN}`, alerts to the in-cluster NTFY `uptime` topic — token in the SOPS-encrypted `gatus-ntfy` secret, interpolated into the Gatus config as `${NTFY_TOKEN}`; the placeholder lives in the `gatus-values` secret consumed via `valuesFrom` because Flux post-build substitution runs in strict mode and would fail on it inline), a Prometheus Pushgateway (`observability/pushgateway`) that receives `renovate-metrics` from the Renovate CronJob (its logs are piped through the binary via an initContainer/shared volume), plus ServiceMonitors for cilium/cnpg/flux/longhorn/adguard/pushgateway, and PrometheusRules (`observability/prometheusrules/`, reconciled by its own Flux Kustomization). Private ops alerts (backup/storage internals) carry the label `channel: ntfy` and Alertmanager routes them via webhook to the `ntfy-alertmanager` bridge (`observability/ntfy-alertmanager/`, kriegalex chart wrapping xenrox/ntfy-alertmanager), which publishes formatted notifications (severity → priority/tags) to the private ntfy `alerts` topic. The bridge config (scfg, holds the ntfy token) is the SOPS-encrypted `ntfy-alertmanager-config` secret; the ntfy `alertmanager` user has write-only access to that topic. Everything else falls through to the null receiver — public/user-facing alerting stays on Gatus → ntfy `uptime`.
...
- **NVR (Frigate)**: `cluster/apps/iot/frigate/`, pinned to `ser-n100wk1` via `nodeSelector` + toleration (see Hardware & topology). Deliberate exceptions to the usual pod conventions: the container is **privileged** and the namespace has `pod-security.kubernetes.io/enforce: privileged` (USB Coral via `/dev/bus/usb` + VA-API via `/dev/dri` hostPaths), and config/DB live on a node-local `hostPath` (`/var/lib/frigate`) instead of Longhorn (Longhorn cannot attach on that node). Recordings go straight to the QNAP over NFS (PV `${NAS_SHARE_PATH}/nvr`, no Longhorn backup label — they are the backup destination themselves). Ports: `5000` pod-internal unauthenticated (probes), `8971` authenticated UI behind the Service; `tls.enabled: false` in `config.yml` because the Gateway terminates TLS. Public UI at `frigate-adm.${CLUSTER_DOMAIN}` behind Authelia (`ExternalAuth` filter on the HTTPRoute + Frigate `proxy.header_map` SSO; role_map group `admins`). Authelia ReferenceGrant note: the CRD caps `spec.from` at 16 entries — `reference-grant.yaml` is full, add new namespaces to `reference-grant-2.yaml`. Object detection runs on two USB Coral TPUs with the YOLOv9-s EdgeTPU model (`model_type: yolo-generic`, fixed 17-class coco17 labelmap); the `.tflite` + labelmap are **not in git** — they are downloaded into `/var/lib/frigate/model_cache/` on the node (see the download URLs in `configmap.yaml`'s model comment / docs.frigate.video object_detectors#yolov9) and must be re-fetched if that hostPath is wiped.
- **GPU**: `ser-msa2cp1` has an NVIDIA RTX A2000 12GB. Drivers/container toolkit are Talos system extensions (`nonfree-kmod-nvidia-lts` + `nvidia-container-toolkit-lts` in `talos/talconfig.yaml`, node patch `talos/patches/nodes/ser-msa2cp1-gpu.yaml` loads the kernel modules and sets the `nvidia.com/gpu.present=true` label); the `nvidia-device-plugin` HelmRelease (`cluster/infrastructure/nvidia-device-plugin/`) exposes `nvidia.com/gpu` to Kubernetes. GPU workloads must request the resource **and** set `runtimeClassName: nvidia` (RuntimeClass created by the same component; the handler is registered by the toolkit extension).
...
- **Variable substitution**: cluster-wide values are referenced as `${VARIABLE_NAME}` and injected via Flux `postBuild.substituteFrom`. Non-sensitive values (storage classes, NIC name, NAS share path) live in the public `cluster/cluster-vars.yaml` ConfigMap; sensitive values (domain, IPs, email) live ONLY in the SOPS-encrypted `cluster/cluster-vars-private-secret.yaml` ConfigMap (`cluster-vars-private`), listed after `cluster-vars` in every `substituteFrom` so its values win. Flux substitutes in strict mode: a variable missing from both ConfigMaps fails the Kustomization loudly. Use these variables instead of hardcoding IPs/domains, and never add real sensitive values to public files.
