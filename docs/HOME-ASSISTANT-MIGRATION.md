...
| Config volume        | NFS `nfs://10.0.4.1/k8s/home-assistant-config-pvc-fb7de188-…` (50 Gi)        | Longhorn RWO PVC `home-assistant-config` (30 Gi, expandable)                                    |
| Ingress              | Ingress on `ha.homme.ar`                                                    | Gateway API `HTTPRoute` on `ha.${CLUSTER_DOMAIN}` behind `envoy-gateway`                        |
| PostgreSQL           | Bitnami chart 18.7.2, PG **18.4**, LB `10.0.11.2`, DB `home-assistant`, user `home-assistant` | Shared CNPG PG18 cluster in `postgres` namespace, DB `homeassistant`, role `homeassistant`      |
| Reachable at         | `postgresql://home-assistant:***@10.0.11.2:5432/home-assistant`             | `postgresql://homeassistant:***@postgres-rw.postgres.svc.cluster.local:5432/homeassistant`      |
...
- Legacy PG has `wal_level=logical`. Verify with:
  `psql -h 10.0.11.2 -U postgres -c 'SHOW wal_level;'`
...
- the legacy NFS export directly (`nfs://10.0.4.1/…`), and
...
Connect as super-user from any pod that can reach `10.0.11.2:5432`
...
During initial COPY the legacy HA keeps writing to `10.0.11.2` and every
...
- Open `https://ha.homme.ar` and check `Settings → System → Repairs`.
...
3. Leave `LEGACY_HA_IP: 10.0.11.5` in `cluster-vars.yaml` for now — some
