...
| Data volume      | NFS `nfs://10.0.4.1/k8s/apps-n8n-data-pvc-…` (50 Gi)   | Longhorn RWO PVC `n8n-data` (20 Gi, expandable; actual usage is far below 50 Gi) |
| Ingress          | Ingress on `n8n-apps.homme.ar`                         | Gateway API `HTTPRoute` on `n8n-apps.${CLUSTER_DOMAIN}` behind `envoy-gateway`   |
| PostgreSQL       | Bitnami PG on `10.0.11.2:5432`, DB `n8n`, role `n8n`   | Shared CNPG PG18 cluster in `postgres` namespace, DB `n8n`, role `n8n`           |
| Reachable at     | `postgresql://n8n:***@10.0.11.2:5432/n8n`              | `postgresql://n8n:***@postgres-rw.postgres.svc.cluster.local:5432/n8n`           |
...
(`10.0.11.2`-style). Today `n8n-apps.homme.ar` resolves to the new gateway
...
or list the exports on the NFS server (`10.0.4.1:/k8s`) and look for
`apps-n8n-data-pvc-*`.
...
`legacy-fallback` wildcard, so `https://n8n-apps.homme.ar` now terminates
on the new pod. No DNS or route changes needed.

### 2.5 Validate

- Open `https://n8n-apps.homme.ar` and log in with the usual owner account.
...
- Fire a webhook-based workflow and confirm the external system receives
  the callback URL `https://n8n-apps.homme.ar/...` (from `WEBHOOK_URL`).
