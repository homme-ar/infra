...
- Preserve `/app/.media` (downloaded media; `WHATSAPP_FILES_LIFETIME=0` keeps
  files forever).
- Keep the same hostname (`waha-adm.example.com`) and the same API key /
...
| Sessions volume  | NFS `192.0.2.20:/k8s/apps-waha-sessions-pvc-…` 10Gi| Longhorn RWO PVC `waha-sessions` (1Gi; actual usage ~150Ki)                    |
| Media volume     | NFS `192.0.2.20:/k8s/apps-waha-media-pvc-…` 100Gi  | Longhorn RWO PVC `waha-media` (5Gi, expandable; actual usage ~4Ki)             |
| Ingress          | Traefik Ingress + authentik forwardAuth          | Gateway API `HTTPRoute` + Authelia ext-authz (all paths, same behavior)        |
| Hostname         | `waha-adm.example.com`                              | `waha-adm.${CLUSTER_DOMAIN}` (identical)                                       |
...
`cluster/apps/platform/waha/httproute.yaml` matches `waha-adm.example.com`
...
dashboard (`https://waha-adm.example.com/dashboard`) and re-scan — the API key
...
`apps-waha-media-pvc-eb182008-2c30-495b-8244-73087aa03a74` on `192.0.2.20`.
...
`legacy-fallback` wildcard, so `https://waha-adm.example.com` now terminates
...
- Open `https://waha-adm.example.com` — Authelia login first (2FA), then the
  WAHA swagger UI behind `admin` / swagger password (unchanged).
- Dashboard at `https://waha-adm.example.com/dashboard` (same credentials as
  legacy).
...
2. Remove the legacy NFS export directories
   (`apps-waha-sessions-pvc-*`, `apps-waha-media-pvc-*` on `192.0.2.20:/k8s`)
