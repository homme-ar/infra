...
Open `https://seerr-apps.example.com`: login works, users/requests/settings intact, Jellyfin + Sonarr/Radarr connections still configured.
...
Old namespace/PVC/volume are untouched. `flux resume kustomization cluster-apps -n flux-system` brings jellyseerr back at `jellyseerr-apps.example.com`; delete the `seerr` namespace and the migrated PV, and revert the git commits.
